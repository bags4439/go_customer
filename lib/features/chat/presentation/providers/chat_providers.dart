...
final messagesProvider =
    StreamProvider.family<MessagesPageResult, String>((ref, orderId) {
...
final chatPaginationNotifierProvider =
    StateNotifierProvider.family<ChatPaginationNotifier, ChatPaginationState, String>(
...
class ChatPaginationNotifier extends StateNotifier<ChatPaginationState> {
  ...
  ChatPaginationNotifier(this._ref, this.orderId) : super(const ChatPaginationState());
  ...
  Future<void> loadMore(DocumentSnapshot lastDocument) async {
    ...
    try {
      ...
      final (next, nextLastDoc) = await ds.loadMoreMessages(orderId, lastDocument);
      state = ChatPaginationState(
...
final pendingVideoPathProvider =
...
final unreadFromAgentCountProvider =
    StreamProvider.family<int, String>((ref, orderId) {
  return ref
      .watch(firestoreProvider)
      .collection(FirestoreCollections.messages)
...
final messageReactionsProvider =
  ...
  return ref
      .watch(chatDataSourceProvider)
      .watchMessageReactions(orderId);
});

final agentTypingProvider =
    StreamProvider.family<bool, String>((ref, orderId) {
...
final replyStateProvider =
    StateProvider.family<ReplyState?, String>((ref, orderId) => null);

/// Combined list: older (pagination) + latest page (stream), then pending (text/image only). Chronological order.
final chatDisplayMessagesProvider =
    Provider.family<List<Object>, String>((ref, orderId) {
  ...
  final combined = <Object>[...fromFirestore, ...pendingOnly];
  combined.sort((a, b) {
...
final chatFirstPageLastDocProvider =
    Provider.family<DocumentSnapshot?, String>((ref, orderId) {
...
Future<void> sendTextMessageForOrder(WidgetRef ref, String orderId) async {
  ...
  ref.read(pendingMessagesProvider(orderId).notifier).state = [
    ...
    PendingMessage(tempId: tempId, messageType: 'text', body: body, sentAt: sentAt),
  ];
  ref.read(messageInputProvider.notifier).state = '';
  ref.read(replyStateProvider(orderId).notifier).state = null;

  try {
    final docId = await ref.read(chatDataSourceProvider).sendTextMessage(
          orderId: orderId,
          senderId: user,
          senderRole: 'buyer',
          body: body,
    ...
    ref.read(pendingMessagesProvider(orderId).notifier).state =
        ref.read(pendingMessagesProvider(orderId)).where((p) => p.tempId != tempId).toList();
  }
}
...
Future<void> sendSelectedImagesForOrder(WidgetRef ref, String orderId) async {
  ...
  for (final path in images) {
    ...
    ref.read(pendingMessagesProvider(orderId).notifier).state = [
      ...ref.read(pendingMessagesProvider(orderId)),
      PendingMessage(
          tempId: tempId, messageType: 'image', localPath: path, sentAt: sentAt),
    ];
    ...
    try {
      ...
      final idx = list.indexWhere((p) => p.tempId == tempId);
      if (idx >= 0) {
        final updated = list[idx].copyWith(status: 'sent', firestoreId: docId, progress: 1.0);
        ref.read(pendingMessagesProvider(orderId).notifier).state = [
          ...list.sublist(0, idx),
          updated,
          ...list.sublist(idx + 1),
...
Future<void> removePendingImage(WidgetRef ref, String orderId, int index) async {
...
Future<void> sendVoiceNoteForOrder(
  ...
  if (user == null) return;
  final reply = ref.read(replyStateProvider(orderId).notifier).state;
  await ref.read(chatDataSourceProvider).sendVoiceNote(
        orderId: orderId,
        senderId: user,
        senderRole: 'buyer',
        file: file,
...
Future<void> sendFileForOrder(WidgetRef ref, String orderId) async {
  ...
  if (user == null) return;
  final reply = ref.read(replyStateProvider(orderId).notifier).state;
  await ref.read(chatDataSourceProvider).sendFileMessage(
        orderId: orderId,
        senderId: user,
        senderRole: 'buyer',
        file: file,
...
Future<void> sendVideoForOrder(WidgetRef ref, String orderId) async {
  ...
  try {
    ...
    await ref.read(chatDataSourceProvider).sendVideoMessage(
          orderId: orderId,
          senderId: user,
          senderRole: 'buyer',
          file: file,
...
Future<void> toggleReactionForMessage(
  ...
  final user = ref.read(authStateProvider).value;
  if (user == null) return;
  await ref.read(chatDataSourceProvider).upsertReaction(
        orderId: orderId,
        messageId: messageId,
        userId: user,
        emoji: emoji,
