...
final orderTimelineProvider =
  ...
  return firestore
      ...
      .map((snapshot) => snapshot.docs
          .map(OrderTimelineModel.fromFirestore)
          .toList());
});
...
final pendingPaymentRequestsProvider =
  ...
  return firestore
      ...
      .map((snapshot) {
    final list =
        snapshot.docs.map(PaymentRequestModel.fromFirestore).toList();
    list.sort((a, b) {
...
final orderShippingProvider =
    StreamProvider.family<ShippingModel?, String>((ref, orderId) {
  final firestore = ref.watch(firestoreProvider);
  return firestore
...
final orderRepairJobProvider =
    StreamProvider.family<RepairJobModel?, String>((ref, orderId) {
  final firestore = ref.watch(firestoreProvider);
  return firestore
      ...
      .map((s) => s.docs.isEmpty
          ? null
          : RepairJobModel.fromFirestore(s.docs.first));
});
